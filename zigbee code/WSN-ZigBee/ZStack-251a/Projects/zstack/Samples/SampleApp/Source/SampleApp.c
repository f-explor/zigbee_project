...
void SampleApp_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if ( keys & HAL_KEY_SW_6 ) //S1 控制正反转
  {
    if (SampleApp_NwkState == DEV_ZB_COORD) 
    {
      ...
      DataBuf[0] = 0x30;   //30为正反转  31为速度
      DataBuf[1] = ucDirection; 
      DataBuf[2] = 0;
      SampleApp_SendPeriodicMessage(); //协调器只发1 电机正转  
    }
    
  }
  
  if ( keys & HAL_KEY_SW_1 ) //S2 控制速度
  {
    if (SampleApp_NwkState == DEV_ZB_COORD) 
    {
      ucSpeed++;      //速度2-10之间
...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  case SAMPLEAPP_PERIODIC_CLUSTERID:       
    ...
    if(buf[0] == 0x30)     //0x30 正反转标记
    {
      ucEdDir = buf[1];    //保存旋转方向给调速用
      MotorStop();         //停止转动
      if(buf[1] == 0x01)    
      {
        for(i=0;i<500;i++)
